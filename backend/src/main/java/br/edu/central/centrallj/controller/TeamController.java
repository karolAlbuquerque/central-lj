...
public class TeamController {
  ...
  public TeamController(ManageEquipeUseCase manageEquipeUseCase) {
  ...
  public ResponseEntity<EquipeResponse> create(@Valid @RequestBody CreateEquipeRequest request) {
    return ResponseEntity.status(HttpStatus.CREATED).body(manageEquipeUseCase.create(request));
  }

  @GetMapping
  public List<EquipeResponse> list() {
    return manageEquipeUseCase.listAll();
  }

  @GetMapping("/{id}")
  public EquipeDetailResponse getDetail(@PathVariable UUID id) {
    return manageEquipeUseCase.getDetail(id);
  }
}
